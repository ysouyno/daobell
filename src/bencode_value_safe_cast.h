  ...
  TO *down_cast(FROM *from)
{
  ...
  // pointed to bencode_dictionary, it will invoke bencode_dictionary's visit
  // funciton, and because parameter 'to' is a derived class inherited from
  // bencode_crawler pointer also, so when call bencode_dictionary's visit
  // funciton, it will call bencode_value_safe_cast's crawl function, and in
  // that function the parameter type is this pointer that is bencode_dictionary
  // class type, see above to continue
